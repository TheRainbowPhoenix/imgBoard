...
if (@$_SESSION['username']) {
  ...
  if((IsOwner(null,$suname)) || ($suname == "admin")) {
    ...
    if(isset($_GET['pcomm']) && !empty($_GET['pcomm']) && (isset($_GET['img']) && !empty($_GET['img']))) {
      ...
      $name = $id;
      if ($PostDetails != false) $name=$PostDetails[0];
      $Mmessage = $_SESSION['username']." commented &nbsp;<a href='".ROOTPATH."Boards/".$BoardUserName."/view.php?img=".$id."'>".$name."</a>";
      HandleEvent(0, $Mmessage,"Boards/".$BoardUserName."/");
      addComment($id,$suname,$Pcomm);
      header('Location: view.php?img='.$id, true, 303);
    }

    if(isset($_GET['Like']) && !empty($_GET['Like']) && (isset($_GET['img']) && !empty($_GET['img']))) {
      $BoardUserName = basename(dirname(__FILE__));
      $BLike = @trim($_GET['Like']);
      if ($BLike=='false') {
        $likeit==true;
        $id = @trim($_GET['img']);
        $name = $id;
        $PostDetails = GetPostDetails(null,$file);
        if ($PostDetails != false) $name=$PostDetails[0];
        $Mmessage = $_SESSION['username']." liked &nbsp;<a href='".ROOTPATH."Boards/".$BoardUserName."/view.php?img=".$id."'>".$name."</a>";
        HandleEvent(1, $Mmessage,"Boards/".$BoardUserName."/");
        header('Location: view.php?img='.$id, true, 303);
      } else {
        $likeit==false;
      }
 ...
 <html>
   ...
   <div class="large-container">
   	...
   	<div class="card midgrey">
          ...
          echo '<a href="?img='.$file.'&Like='.(($likeit) ? 'true' : 'false').'"
					class="right btn-second btn btn-lg btn-primary btn-block" title="Like this content">'.$SvgHeart.'</a> '; ?>
