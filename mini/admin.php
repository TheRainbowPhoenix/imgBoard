...
if(strpos($_SESSION['username'], "admin")!== false) {
	...
	if(isset($_GET['addusername']) && !empty($_GET['addusername']) && isset($_GET['addemail']) && !empty($_GET['addemail']) && isset($_GET['addpassword']) && !empty($_GET['addpassword'])) {
	  $addusername = @trim($_GET['addusername']);
		...
		if (addUser("Users.json", $addusername, $addemail,$addpassword)) {
							...
							<p id="userparser">
									...
									foreach ($jsonRS as $rs) {
										if (! $rs["name"] == "") {
											...
											echo '<a href="'.'/mini/Boards/'.stripslashes($rs["name"]).'/"class="list-item-title">'.stripslashes($rs["name"]).'</a>';
						...
						<div class="card-content">
							<p id="boardsparser">
									...
									foreach ($dirs as $rs) {
										$bname = explode("/", $rs);
										echo '<li class="list-item"><div class="list-content">';
										echo '<a href="'.'/mini/'.$rs.'/"class="list-item-title">'.end($bname).'</a>';
										if (isPresent($filename, end($bname))) {
											echo '<p class="list-item-desc">Owned by '.'<a href="'.'/mini/Boards/'.end($bname).'/">'.end($bname).'</a></p>';
										} else {
					...
					<div class="card midgrey">
						...
						<div class="card-content">
							<p id="commentsparser">
								...
								$jsonRS = json_decode ($string,true);
								echo '<span class="input-info" id="basic-addon1">Filter :</span>';
								echo '<input type="text" name="ListFilter" id="clistFilter" class="textfield" onkeyup="doFilter(';
								echo "'clistFilter', 'clist', true)";
								echo '" title="Input something to filter the following list">';
					...
					<div class="card midgrey">
						...
						<div class="card-content">
							<p id="commentsparser">
								...
								echo "'/mini/admin.php', {CLog: true}, 'get'";
								...
								echo "'/mini/admin.php', {ELog: true}, 'get'";
								...
								foreach ($mainlog  as $line) {
									$line_details = explode('|', $line);
										...
										echo '</div>';
										echo '<span title="View details about this log entry"><svg id="editUser" viewBox="0 0 24 24"><path class="editpen" fill="rgba(210,210,210,.9)" d="M16.95 2.8l-.2.2-.5.52-2.13 2.12-.7.7L3 16.76V21h4.24l1-1 9.42-9.4.7-.72 2.12-2.12.52-.52.2-.2-.2-.18-.52-.52-2.82-2.82-.52-.52-.2-.2zm0 1.42l2.83 2.83-2.12 2.12-2.83-2.83 2.12-2.12zm-2.83 2.83l2.83 2.83L9 17.83V17H7v-2h-.83l7.95-7.95zM5.17 16H6v2h2v.83L6.83 20H5.15L4 18.85v-1.68L5.17 16z"/></svg></span>';
										echo '</li>';
								}
								echo "</ul>";
						...
						<div class="card-content">
							...
							foreach ($jsonRS as $rs) {
								if (! $rs["name"] == "") {
									...
									echo "'/mini/Boards/".stripslashes($rs["name"])."','_blank'";
							...
							<div class="input-group">
								...
								<input type="text" name="nusername" class="textfield" autofocus="autofocus">
							</div>
							<div class="input-group">
								<span class="input-info" id="basic-addon1">new mail:</span>
								<input type="text" name="nemail" class="textfield">
							</div>
							<div class="input-group">
								<label for="inputPassword" class="input-info sr-only">new password :</label>
								<input type="password" id="inputPassword" class="textfield" onblur="hashPsw()">
...
function hidemodals() {
	...
	var it = modals.length-1;
	for (it; it >=0; --it) {
		if (modals[it].style.display == "block") {
			document.getElementsByClassName("modal")[it].style.display = "none";
			//if (it == 0) document.body.style.overflow = 'auto';
			return;
		}
	}
}
...
function rmuser(uname = null) {
	...
	post('/mini/admin.php', {rmuser: uname}, "get");
}

function uexp(uname) {
	post('/mini/admin.php', {uexp: uname}, "get");
}
...
function cleanthisDB(DBname) {
	...
	post('/mini/admin.php', {ClDB: RealDBname}, "get");
}
